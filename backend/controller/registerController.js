...
const registerController=async (req,res)=>{
  try {
   ...
   // console.log('req.body', req.body)
    if(!name||!email|!phone||!password||!address||!role||!question){
    ...
    if(uniqueEmail){
        ...
        const newUserData = new userModel({ name, email,phone,password:hashPassword,address,role,question});
  ...
  try {
    if(!email||!question){
      ...
      if(user.email==email&&user.question==question){
        const hashPassword =await hashpass(password);
        const updatePass=await userModel.findByIdAndUpdate(user._id,{password:hashPassword})
